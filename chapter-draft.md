...
### 2.1 Template matching and reference-based picking

The classical picking baselines split into two related but distinct families. The first is *reference-based auto-picking* in RELION and cryoSPARC — both packages let the user supply a small set of 2D class averages (typically generated from a hand-picked seed set) and cross-correlate them against each micrograph; peaks above a threshold become particles (Scheres, 2015). This is what is colloquially called "template matching" in SPA workflows, but it is not the same operation as the high-resolution 2D template matching used to locate complexes inside cellular cryo-EM data. RELION supports both SPA and subtomogram averaging; cryoSPARC is SPA-only. Gautomatch is a long-standing GPU implementation in the same family, widely used in practice but without a peer-reviewed publication.

The modern reference for high-resolution 2D template matching is **cisTEM 2DTM** (Lucas et al., 2021), which projects an atomic-model template at high angular sampling and detects matches against a statistically calibrated noise floor. cisTEM 2DTM is now the more common choice when an atomic model is available and rigorous statistical detection is needed — including for locating single complexes inside cellular cryo-EM data, where RELION/cryoSPARC reference-based picking would not work.

Across both families, the main failure mode is template bias, often called the *Einstein-from-noise* problem: a picker that correlates against a particular reference will preferentially find that reference even in pure noise, biasing the downstream reconstruction toward the starting model. These approaches are therefore a reasonable starting point for known targets and a poor starting point for unknown ones.
...
**CEM500K** (Conrad et al., 2021), discussed in §5, is the canonical curated pretraining dataset for vEM: 500,000 images spanning multiple modalities and tissues, along with pretrained encoders that now form the standard backbone for fine-tuning vEM segmentation. The **CryoET Data Portal** (Ermel et al., 2024, *bioRxiv*), maintained by the Chan Zuckerberg Imaging Institute, plays the analogous role for cryo-electron tomography: open access to thousands of curated tilt series and tomograms with growing annotation coverage. Both build on top of the broader EM data infrastructure — **EMPIAR** (Iudin et al., 2016) for raw datasets and **EMDB** (Lawson et al., 2024) for reconstructed maps and tomograms — which together constitute the foundational substrate for EM machine learning. **BioImage Model Zoo** (Ouyang et al., 2022, *bioRxiv*) is the cross-community model-sharing infrastructure for bioimage analysis, including but not limited to EM, and provides a common format for distributing trained networks across communities. The CryoET Data Portal and Model Zoo are cited as preprints; refresh before submission.
